...
A "moving average" is described as a sliding window of a specific length that efficiently smooths signals by removing noise while maintaining the original properties.
Equations 3.3 and 3.5 are known as moving averages because they both are 3-point windows that calculate the mean of the three most recent samples. 
...
\end{figure}
In Figure 3, the integrator system is unstable because there exists at least one bounded input that results in an unbounded output as seen in subplot 5.
...
Even in subplot 6, the derivative of the unit step function is bounded to a dirac delta function.
...
The S1 filter from Section 1.3 produces a higher pitch and the S2 filter produces a lower pitch.
...
Moreover, lower frequency pass through and are heard in the audio.
...
\end{figure}
In Figure 12, subplot 2 using Equation 3.4 presents a smooth curve that is ideal for finding long-term trends in the stock market.
However, subplot 2 is not accurate for analyzing the sporadic, short-term behavior of the stock market rates.
